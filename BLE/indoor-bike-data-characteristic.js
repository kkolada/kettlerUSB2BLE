...
class IndoorBikeDataCharacteristic extends Bleno.Characteristic {
  ...
  onSubscribe(maxValueSize, updateValueCallback) {
    if (DEBUG) {
      console.log('[IndoorBikeDataCharacteristic] client subscribed');
    }
  ...
  onUnsubscribe() {
    if (DEBUG) {
      console.log('[IndoorBikeDataCharacteristic] client unsubscribed');
    }
    this._updateValueCallback = null;
  ...
  notify(event) {
    ...
    if (this._updateValueCallback) {
      if (DEBUG) {
        console.log('[IndoorBikeDataCharacteristic] Notify');
      }
      ...
      if ('speed' in event) {
        ...
        if (DEBUG) {
          console.log('[IndoorBikeDataCharacteristic] speed: ' + speed);
        }
      ...
      if ('rpm' in event) {
        var rpm = event.rpm;
        if (DEBUG) {
          console.log('[IndoorBikeDataCharacteristic] rpm: ' + rpm);
        }
      ...
      if ('power' in event) {
        ...
        if (DEBUG) {
          console.log('[IndoorBikeDataCharacteristic] power: ' + power);
        }
      ...
      if ('hr' in event) {
        var hr = event.hr;
        if (DEBUG) {
          console.log('[IndoorBikeDataCharacteristic] hr : ' + hr);
        }
      ...
      if (DEBUG) {
        console.log("[IndoorBikeDataCharacteristic] nobody is listening");
      }
    }
    return this.RESULT_SUCCESS;
  }
